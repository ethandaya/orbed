...
test('CLI exit codes distinguish pass, blocked gate and malformed input', async () => {
  ...
  try {
    ...
    for (const [report, expected] of [[base, 0], [{ ...base, complete: false, passed: false }, 1],
      [{ ...base, passed: false, results: [{ name: 'case', status: 'failed' }] }, 1],
      [{ ...base, results: [{ name: 'case', status: 'failed' }] }, 2],
      [{ ...base, error: '' }, 2], [{}, 2]]) {
    ...
    const invalid = spawnSync('node', ['dist/cli.js', 'run'], { encoding: 'utf8' })
    assert.equal(invalid.status, 2)
    assert.match(invalid.stderr, /orbed check/)
