...
import { parseReport, runRemote, exitCode } from './remote.js'
...
try {
  ...
  if (values.help) {
    console.log('orbed run --project namespace/project --revision <full-sha> [--output report.json] [--timeout-ms 900000]\norbed check <report.json>\nExit: 0 passed; 1 failed/incomplete; 2 invocation or transport error.')
    ...
    const report = await runRemote(values.project, values.revision, AbortSignal.timeout(timeoutMs))
